...
StatDensityRidges <- ggproto("StatDensityRidges", Stat,
  ...
  compute_group = function(data, scales, from, to, bandwidth = 1,
    ...
    if (calc_ecdf) {
      ...
      rbind(data.frame(x = d$x, density = d$y, datatype = "ridgeline",
                       stringsAsFactors = FALSE), df_quantiles, df_jittered)
    }
  }
)
